...
class Item:
    ...
    # A class attribute
    nb_items = 0
    current_datetime = datetime.datetime.now().strftime("%A %d %B %Y at %H:%M:%S")

    def __init__(self, name, price, quantity):
        """ Initializes class attributes"""
        self.__id = str(uuid.uuid4())
        self.__time_entered = str(datetime.datetime.now().strftime("%A %d %B %Y at %H:%M:%S"))
        self.name = name
        self.price = price
